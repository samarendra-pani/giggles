...
void GenotypeHMM::compute_backward_prob()
{
	...
	for(int column_index = column_count-1; column_index >= 0; --column_index){
		...
		if (column_index > 0) {
			transition_probability_table[column_index - 1] = new TransitionProbabilityComputer(recombcost[column_index-1], variant_info_table->at(column_index).allele_references);
		}
...
void GenotypeHMM::compute_backward_column(size_t column_index, unique_ptr<vector<const Entry*>> current_input_column) {
	...
	int n_alleles = variant_info_table->at(column_index).active_alleles.size();
	...
	if (column_index > 0) {
		...
		while (iterator->has_next()){
			...
			vector<int> haplotype_to_allele_prev = variant_info_table->at(column_index).allele_references;     // This contains the haplotype-to-allele mapping for the position column_index
			vector<int> haplotype_to_allele_curr = variant_info_table->at(column_index-1).allele_references;     // This contains the haplotype-to-allele mapping for the position column_index-1
			...
			for (int i = 0; i < compatible_bipartitions.size(); i++) {
				...
				for (int r_index = 0; r_index < pow(n_references,2); r_index++) {
					...
					current_projection_column->at(index+r_index) += pow(current_transition_table->get_qr(),2)*beta_helper_0 + current_transition_table->get_qr()*current_transition_table->get_pr()*(beta_helper_2[ref[0]]+beta_helper_3[ref[1]]-(2*beta_helper_0)) + pow(current_transition_table->get_pr(),2)*(beta_helper_1-beta_helper_2[ref[0]]-beta_helper_3[ref[1]]+beta_helper_0);
				}
			}
		}
	}
...
void GenotypeHMM::compute_forward_column(size_t column_index, unique_ptr<vector<const Entry*>> current_input_column)
{
	...
	if (column_index > 0) {
		haplotype_to_allele_prev = variant_info_table->at(column_index-1).allele_references;     // This contains the haplotype-to-allele mapping for the position column_index-1
	}
		...
		if (column_index == 0) {
			...
			for (unsigned int b : compatible_bipartitions) {
				...
				for (int r_index = 0; r_index < pow(n_references, 2); r_index++) {
					...
					current_projection_column->at(index) += emission_probability_computer.at(allele_curr[0], allele_curr[1])*(pow(current_transition_table->get_qr(),2)*alpha_helper_0 + current_transition_table->get_qr()*current_transition_table->get_pr()*(alpha_helper_2[ref[0]]+alpha_helper_3[ref[1]]-(2*alpha_helper_0)) + pow(current_transition_table->get_pr(),2)*(alpha_helper_1-alpha_helper_2[ref[0]]-alpha_helper_3[ref[1]]+alpha_helper_0));
				}
			}
		}
	}
	...
	for (unsigned int i = 0; i < backward_probabilities->size(); i++) {
		...
		variant_info_table->at(column_index).genotype_likelihoods.increment_by_index(g_index, forward_backward);
	}
		...
		if (newBit == 1) {
			for (int i = 0; i < n_alleles; i++) {
				for (int j = 0; j < n_alleles; j++) {
					...
					long double new_value = em_prob->at(i, j) * (entries.at(bit_changed)->get_scores()[j]/entries.at(bit_changed)->get_scores()[i]);
			...
			for (int i = 0; i < n_alleles; i++) {
				for (int j = 0; j < n_alleles; j++) {
					...
					long double new_value = em_prob->at(i, j) * (entries.at(bit_changed)->get_scores()[i]/entries.at(bit_changed)->get_scores()[j]);
					em_prob->set(i, j, new_value);
				}
			}
		}
			...
			for (int j = 0; j < n_alleles; j++) {
				...
				for (int entry_index = 0; entry_index < entries.size(); entry_index++) {
					...
					if (binaryVector[entry_index] == 0){
						...
						value = value * (entries.at(entry_index)->get_scores()[i]);
					}
					else {
						...
						value = value * (entries.at(entry_index)->get_scores()[j]);
					}
				}
