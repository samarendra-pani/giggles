...
void EmissionProbabilityComputer::update_emission_probability(const int cluster_bit_changed, const BipartitionIterator& iterator, std::vector<const Entry *>& entries) {
	...
	if (cluster_bit_changed >= 0) {
		 ...
		 */
		std::unordered_map<uint32_t, bool> changed_reads = iterator.get_changed_reads(cluster_bit_changed);
		for (auto const& [entry_index, newBit]: changed_reads) {
			...
			assert(entries.at(entry_index)->get_read_id() == iterator.get_parent_column()->get_read_ids()->at(entry_index));
			if (entries.at(entry_index)->get_allele_type() == Entry::BLANK) {
				/**
			...
			for (uint32_t i = 0; i < n_alleles; i++) {
				for (uint32_t j = 0; j < n_alleles; j++) {
					...
					if (newBit) {
						 ...
						 */
						numerator = entries.at(entry_index)->get_emission_scores()[j];
						denominator = entries.at(entry_index)->get_emission_scores()[i];
					} 
					else {
						/**
						 ...
						 */
						numerator = entries.at(entry_index)->get_emission_scores()[i];
						denominator = entries.at(entry_index)->get_emission_scores()[j];
					}
					long double new_value = emission_probability_table.at(i, j) * (numerator/denominator);
					emission_probability_table.set(i, j, new_value);
				}
			}
		}	
	}
		 ...
		 */
		uint32_t index = iterator.get_read_cluster_bit_representation(); // this is value showing the bipartition of every read cluster.
		/**
		...
		for (uint32_t i = 0; i < n_alleles; i++) {
			for (uint32_t j = 0; j < n_alleles; j++) {
				 ...
				 * The bits of index correspond to these cluster ids.
				 */
				std::vector<uint32_t>* cluster_ids = iterator.get_parent_column()->get_read_cluster_ids();
				for (uint32_t c_id: *cluster_ids) {
					/**
					 ...
					 */
					std::vector<uint32_t> read_indices = iterator.get_read_index_from_cluster_id(c_id);
					// Determine whether the current cluster is in bipartition 0 or 1
					bool bit = (index & (1 << cluster_count)) != 0;
					for (uint32_t r_idx: read_indices) {
						if (entries.at(r_idx)->get_allele_type() == Entry::BLANK) {
							continue;
						}
						if (bit) {
							// If read at r_idx is in biparition 1 then value gets multiplied with emission from allele j
							value = value * (entries.at(r_idx)->get_emission_scores()[j]);
						}
						else {
							// If read at r_idx is in biparition 0 then value gets multiplied with emission from allele i
							value = value * (entries.at(r_idx)->get_emission_scores()[i]);
						}
					}
