...
GenotypingAlgorithm::GenotypingAlgorithm(ReadSet* read_set, const std::vector<float>& recombcost, const unsigned int& n_references, const std::vector<unsigned int>* positions, const std::vector<unsigned int>* n_allele_positions, const std::vector<std::vector<int> >* allele_references, const std::vector<bool>* is_sv_position) {
	...
	for (size_t i = 0; i < positions->size(); i++) {
		variant_info_table[i] = variant_information_t(positions->at(i), n_allele_positions->at(i), allele_references->at(i), is_sv_position->at(i));
