...
private:
	...
	std::vector<uint32_t> untagged_read_ids;
	// read IDs of active reads at the next variant position
	std::vector<uint32_t> next_read_ids;
	// read IDS of untagged active reads at the next variant position
	std::vector<uint32_t> untagged_next_read_ids;
	// read IDs of active, non terminating reads at the variant position
	std::vector<uint32_t> act_nonterminating_read_ids;
	// read IDs of active, terminating reads at the variant position
	std::vector<uint32_t> act_terminating_read_ids;
	uint32_t n_references;
	
public:

	Column(const uint32_t index, const uint32_t* n_ref, const std::vector<uint32_t>& read_ids, const std::vector<uint32_t>& next_read_ids, ReadSet* set);
	
	// returns the column size
	uint32_t get_column_size();
	...
	std::vector<uint32_t> * get_active_nonterminating_read_ids();

	// return a pointer to the active terminating read ids of the column
	std::vector<uint32_t> * get_active_terminating_read_ids();

	// return a pointer to the active non terminating read ids of the column
	std::vector<uint32_t> * get_next_read_ids();
	...
	uint32_t get_index(uint32_t b_index, uint32_t r_index);

	// returns the compatible bipartitions of bipartition b_index (of pos v+1) in column v
	std::vector<uint32_t> get_backward_compatible_bipartitions(int b_index, ReadSet* set);
